      ...
      <header className="flex items-start justify-between gap-6 flex-wrap">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1.5">
            Insights, recommandations, risques détectés et résumés exécutifs générés par Nexora.
          </p>
        </div>
...
function RecommendationsTab() {
  ...
  if (isLoading) return <Skeleton className="h-40 w-full" />;
  if (!data?.length) return <EmptyState icon={Sparkles} title="Aucune recommandation active" body="Les prochaines meilleures actions suggérées par Nexora apparaîtront ici." />;
  return (
    <div className="grid gap-3">
      {data.map((r) => (
        <Card key={r.id}>
