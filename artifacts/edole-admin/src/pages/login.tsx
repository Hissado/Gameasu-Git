...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen w-full flex flex-col items-center justify-between px-4 py-8"
      style={{ background: "#060D1A", fontFamily: "var(--app-font-display, system-ui)" }}>
      ...
      <div className="w-full max-w-[420px]">
        ...
        {step === "credentials" && (
          <div className="rounded-2xl bg-white px-8 py-8" style={{ boxShadow: "0 4px 32px rgba(0,0,0,0.35)" }}>
            <h1 className="text-[22px] font-bold text-center text-[#0E1A39] mb-6" style={{ letterSpacing: "-0.02em" }}>
              Connexion
            </h1>
              ...
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                <FormField control={form.control} name="email" render={({ field }) => (
                  <FormItem className="space-y-1.5">
                    <FormLabel className="text-[12px] font-medium text-[#374151]">Adresse e-mail</FormLabel>
                    <FormControl>
                      <Input placeholder="prenom.nom@entreprise.com" autoComplete="email"
                        className="h-10 rounded-lg text-[13.5px] border-gray-300 focus-visible:ring-blue-600"
                        {...field} />
                    </FormControl>
                ...
                <FormField control={form.control} name="password" render={({ field }) => (
                  <FormItem className="space-y-1.5">
                    <FormLabel className="text-[12px] font-medium text-[#374151]">Mot de passe</FormLabel>
                    <FormControl>
                      <div className="relative">
                        <Input type={showPassword ? "text" : "password"} placeholder="••••••••••"
                          autoComplete="current-password"
                          className="h-10 pr-10 rounded-lg text-[13.5px] border-gray-300 focus-visible:ring-blue-600"
                          {...field} />
                ...
                <div className="pt-1">
                  <button type="submit" disabled={isLoading}
                    className="w-full h-10 rounded-full text-[13.5px] font-semibold text-white flex items-center justify-center gap-2 transition-all duration-200 active:scale-[0.985] disabled:opacity-60"
                    style={{ background: "#2563EB", boxShadow: "0 2px 12px rgba(37,99,235,0.40)" }}>
                    {isLoading
                ...
                <div className="flex items-center justify-between">
                  <label className="flex items-center gap-2 cursor-pointer text-[12px] text-gray-500">
                  ...
                  <button type="button"
                    ...
                    className="text-[12px] text-blue-600 hover:underline transition-colors"
            ...
            {import.meta.env.DEV && (
              <div className="mt-5 border-t border-gray-100 pt-4">
                <button type="button" onClick={() => setShowDemo((v) => !v)}
                  className="w-full flex items-center justify-between text-[11.5px] text-gray-400 hover:text-gray-600 transition-colors">
                  ...
                  <div className="mt-2 space-y-1">
                    {DEMO_ACCOUNTS.map(({ role, email, password }) => (
                      <button key={email} type="button" onClick={() => fillDemo(email, password)}
                        ...
                        <div>
                          <p className="text-[11.5px] font-semibold text-gray-700">{role}</p>
                          <p className="text-[10px] font-mono text-gray-400">{email}</p>
                        </div>
                        <span className="text-[10px] font-bold text-blue-600 opacity-0 group-hover:opacity-100 transition-opacity">Utiliser →</span>
                      </button>
                    ))}
                  </div>
                )}
            ...
            {forgotSent ? (
              ...
              <form onSubmit={onForgotSubmit} className="space-y-4">
                ...
                <button type="submit" disabled={forgotLoading}
                  className="w-full h-10 rounded-full text-[13.5px] font-semibold text-white flex items-center justify-center gap-2 transition-all duration-200 disabled:opacity-60"
                  style={{ background: "#2563EB", boxShadow: "0 2px 12px rgba(37,99,235,0.40)" }}>
      ...
      {/* ══ PIED DE PAGE ════════════════════════════════════════════════════ */}
      <p className="text-center text-[10.5px] pb-2" style={{ color: "rgba(255,255,255,0.20)" }}>
