...
export default function Dashboard() {
  ...
  const { widgets, isEnabled, saveWidgets, resetWidgets, isSaving } = useDashboardConfig();
  ...
  // ── Rendu ordonné des widgets ──────────────────────────────────────────────
  const renderWidget = (id: string) => {
    switch (id) {
      case "kpis":
        return (
          <section key="kpis" data-tour="dash-kpis">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 gap-4 mt-4">
              <KpiCard label="Encaissements du mois" value={loadingKpis ? "—" : formatFCFACompact(monthlyRevenue)} sub="Paiements reçus" icon={TrendingUp} accent={monthlyRevenue > 0 ? "green" : "default"} loading={loadingKpis} href="/paiements" />
              <KpiCard label="Créances ouvertes" value={loadingKpis ? "—" : formatFCFACompact(outstanding)} sub="À encaisser" icon={Receipt} accent={outstanding > 0 ? "amber" : "green"} loading={loadingKpis} href="/factures" />
              <KpiCard label="Pipeline CRM" value={loadingKpis ? "—" : formatFCFACompact(pipeline)} sub="Opportunités qualifiées" icon={Briefcase} accent="blue" loading={loadingKpis} href="/crm" />
              <KpiCard label="Taux de recouvrement" value={loadingKpis ? "—" : `${collectionRate}%`} sub="Encaissé / Facturé" icon={CheckCircle2} accent={collectionRate >= 75 ? "green" : collectionRate >= 40 ? "amber" : "red"} loading={loadingKpis} />
              <KpiCard label="Projets actifs" value={loadingKpis ? "—" : kpis?.activeProjects ?? 0} sub={`sur ${kpis?.totalProjects ?? 0} au total`} icon={FolderKanban} accent="default" loading={loadingKpis} href="/projets" />
              <KpiCard label="Collaborateurs" value={loadingKpis ? "—" : kpis?.totalCollaborators ?? 0} sub="Effectif actif" icon={Users} accent="default" loading={loadingKpis} href="/collaborateurs" />
              <KpiCard label="Factures en attente" value={loadingKpis ? "—" : overdueInvoices.length} sub={overdueInvoices.length > 0 ? "Relances nécessaires" : "Aucune en retard"} icon={FileText} accent={overdueInvoices.length > 0 ? "red" : "green"} loading={loadingKpis} href="/factures" />
              <KpiCard label="Alertes actives" value={loadingKpis ? "—" : totalAlerts} sub={totalAlerts > 0 ? "Actions requises" : "Tout est à jour"} icon={AlertTriangle} accent={totalAlerts > 3 ? "red" : totalAlerts > 0 ? "amber" : "green"} loading={loadingKpis} />
            </div>
          </section>
        );
      ...
      case "alerts":
        return (
          <section key="alerts" data-tour="dash-alerts" className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {/* Alertes financières */}
            <div>
              <SectionHeader title="Alertes prioritaires" sub="Actions financières urgentes"
                action={<Button variant="ghost" size="sm" className="text-xs" asChild><Link href="/factures">Voir tout <ChevronRight className="w-3.5 h-3.5 ml-0.5" /></Link></Button>}
              />
            ...
            {/* Tâches en retard + à venir */}
            <div>
              <SectionHeader title="Tâches en retard" sub="À traiter en priorité"
                action={<Button variant="ghost" size="sm" className="text-xs" asChild><Link href="/tasks">Voir tout <ChevronRight className="w-3.5 h-3.5 ml-0.5" /></Link></Button>}
              />
              <div className="mt-3 space-y-2">
      ...
      case "chart":
        return (
          <section key="chart" className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            {/* Pipeline CRM */}
            <Card className="shadow-sm border-slate-200" style={{ background: "linear-gradient(135deg, #0f172a 0%, #1e293b 100%)" }}>
      ...
      case "projects":
        return (
          <section key="projects" className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            {/* Projets actifs */}
            <div className="lg:col-span-2">
              <SectionHeader title="Projets actifs" sub="Projets en cours par valeur"
                action={<Button variant="ghost" size="sm" className="text-xs" asChild><Link href="/projets">Tous les projets <ChevronRight className="w-3.5 h-3.5 ml-0.5" /></Link></Button>}
              />
              <div className="mt-3 space-y-2">
                {activeProjects.length === 0 ? (
                  <Card className="shadow-sm border-slate-200"><CardContent className="py-10 text-center text-sm text-muted-foreground">Aucun projet actif.</CardContent></Card>
                    ...
                    <Link key={p.id} href={`/projets/${p.id}`}
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between gap-4 mb-1.5">
                          ...
                          <p className="text-sm font-bold text-slate-700 shrink-0 tabular-nums">{formatFCFACompact(Number(p.budget || 0))}</p>
                        </div>
            ...
            <div>
              ...
              <div className="mt-3 space-y-1">
                {!activity || (activity as any[]).length === 0 ? (
                  <Card className="shadow-sm border-slate-200"><CardContent className="py-10 text-center text-sm text-muted-foreground">Aucune activité récente.</CardContent></Card>
