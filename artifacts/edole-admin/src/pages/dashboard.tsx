...
function MiniCalendar({ events }: { events: { dayOfMonth: number | null; name: string }[] }) {
  ...
  // Monday-first: shift Sunday from 0 → 6
  const startOffset = (firstDow + 6) % 7;

  const eventDays = new Set(
    events
      .filter((e) => {
        if (!e.dayOfMonth) return false;
        // If viewing current month/year, show events this month
  ...
  return (
    <div className="select-none">
      ...
      <div className="grid grid-cols-7 gap-0">
        {DOW.map((d, i) => (
          <div key={i} className="text-center text-[10px] font-bold text-slate-400 pb-1.5">
            {d}
          </div>
        ))}
        {cells.map((d, i) => (
...
// ─────────────────────────────────────────────────────────────────
// Gauge (clôture du mois)
// ─────────────────────────────────────────────────────────────────

function MonthCloseGauge({ daysLeft }: { daysLeft: number }) {
  const daysInMonth = new Date(new Date().getFullYear(), new Date().getMonth() + 1, 0).getDate();
  const pct = Math.max(0, Math.min(1, daysLeft / daysInMonth));
  const r = 48;
  const cx = 60;
  const cy = 60;
...
export default function Dashboard() {
  ...
  // Dates clés du mois (15 = cotisations, 10 = impôts)
  const now = new Date();
  ...
  // Monthly hires formatted for bar chart
  ...
  const birthdaysUpcoming = birthdaysThisMonth.filter((b: any) => b.dayOfMonth !== todayDay && b.dayOfMonth > todayDay);
  ...
  return (
    <div className="space-y-5">
      ...
      <div>
        ...
        <p className="text-sm text-slate-500 mt-0.5 capitalize">{fullDateFr()}</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div className="rounded-2xl p-5 shadow-sm" style={{ background: "linear-gradient(135deg, #0f4c5c 0%, #0e7490 100%)" }}>
          <div className="text-xs font-semibold uppercase tracking-widest text-cyan-200 flex items-center gap-1.5">
            Coût salarial global
          </div>
          <div className="mt-2">
          ...
          <div className="mt-2 text-xs text-cyan-300">
            Masse salariale brute · {new Date().toLocaleDateString("fr-FR", { month: "long", year: "numeric" })}
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
              <div className="flex items-center justify-between mb-1">
                ...
                {charges?.period && (
                  <span className="text-[10px] text-slate-400 font-medium">{charges.period}</span>
                )}
              </div>
              {!charges ? (
                ...
                <>
                    ...
                    <div className="flex-1 min-w-0 space-y-2">
                      <div>
                        ...
                        <div className="text-sm font-bold text-slate-800 mt-0.5">
                          {formatFCFACompact(charges.chargesSalariales)}
                        </div>
                      </div>
                      <div>
                        <div className="flex items-center gap-1.5">
                        ...
                        <div className="text-sm font-bold text-slate-800 mt-0.5">
                          {formatFCFACompact(charges.chargesPatronales)}
                        </div>
                      </div>
                    </div>
                  </div>
                  <div className="mt-3 pt-3 border-t border-slate-100">
                    ...
                    <div className="text-xl font-extrabold text-slate-900 mt-0.5">
                      {formatFCFA(charges.totalCharges)}
                    </div>
                  </div>
                </>
              )}
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 flex flex-col items-center justify-between">
              <div className="w-full">
                <span className="font-semibold text-slate-800 text-sm flex items-center gap-2">
                  ...
                  Clôture du mois de {new Date().toLocaleDateString("fr-FR", { month: "long", year: "numeric" })}
                </span>
              </div>
              <div className="my-4">
              ...
              <Link href="/payroll" className="w-full">
                <Button
                  variant="outline"
                  className="w-full border-cyan-700 text-cyan-700 hover:bg-cyan-50 text-sm font-semibold"
                >
                  Clôturer votre paie
                </Button>
              </Link>
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                      <PieChart>
                        <Pie
                          ...
                          strokeWidth={2}
                        >
                          {ageDistribution.map((_: any, i: number) => (
                            <Cell key={i} fill={AGE_COLORS[i % AGE_COLORS.length]} />
                          ))}
                        </Pie>
                        <RTooltip
                          formatter={(v: any, n: any) => [`${v} collab.`, n]}
                ...
                <div style={{ height: 150 }}>
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={hiresData} barSize={20}>
                      ...
                      <RTooltip
                        formatter={(v: any) => [`${v} embauche(s)`, "Embauches"]}
                        contentStyle={{ fontSize: 12, borderRadius: 8 }}
                      />
        ...
        <div className="space-y-5">
                        ...
                        <div className="flex items-center gap-3 py-2 hover:bg-slate-50 rounded-lg px-1 transition-colors cursor-pointer">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="text-[10px] text-slate-400">
                              Le {b.dayOfMonth} {new Date().toLocaleDateString("fr-FR", { month: "long" })}
                            </div>
                          </div>
                        </div>
                      </Link>
          ...
          {/* Contrats expirant bientôt – résumé */}
