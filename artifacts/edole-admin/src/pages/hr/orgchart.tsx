...
function CollabCard({ collab, highlight }: { collab: Collaborator; highlight: string }) {
  ...
  return (
    <div className={`flex items-center gap-2 p-2 rounded-lg border transition-all hover:shadow-sm ${matches ? "border-primary bg-primary/5" : "border bg-card hover:border"}`}>
      ...
      <div className="min-w-0">
        ...
        <p className="text-xs text-muted-foreground/60 truncate">{collab.jobTitle ?? "—"}</p>
      </div>
    </div>
  );
}
...
function DepartmentNode({ dept, members, color, search }: { dept: Department; members: Collaborator[]; color: string; search: string }) {
  ...
  const filtered = search
    ...
    : members;
  return (
    <div className="rounded-xl border border bg-card shadow-sm overflow-hidden" style={{ borderTopColor: color, borderTopWidth: 3 }}>
      <button className="w-full flex items-center justify-between px-4 py-3 bg-muted/70 hover:bg-muted/70 transition-colors" onClick={() => setOpen(!open)}>
      ...
      {open && (
        <div className="p-3">
          {filtered.length === 0
            ? <p className="text-xs text-muted-foreground/60 text-center py-3">{search ? "Aucun résultat" : "Aucun collaborateur actif"}</p>
...
function UndepartedSection({ collabs, search }: { collabs: Collaborator[]; search: string }) {
  ...
  return (
    <div className="rounded-xl border border-dashed border bg-muted/50 overflow-hidden">
      <button className="w-full flex items-center justify-between px-4 py-3 hover:bg-muted/50 transition-colors" onClick={() => setOpen(!open)}>
        <div className="flex items-center gap-2">
          <div className="w-3 h-3 rounded-full bg-slate-300" />
          <span className="font-medium text-sm text-muted-foreground">Non affectés à un département</span>
          <Badge variant="outline" className="text-xs h-4 px-1.5 text-muted-foreground/60">{collabs.length}</Badge>
        </div>
        {open ? <ChevronDown className="w-3.5 h-3.5 text-muted-foreground/60" /> : <ChevronRight className="w-3.5 h-3.5 text-muted-foreground/60" />}
      </button>
...
function ManagerTreeNode({ manager, children, allCollabs, search, depth }: {
  ...
  return (
    <div className={`${depth > 0 ? "ml-6 border-l-2 border pl-4" : ""}`}>
      <div className={`flex items-start gap-3 py-2 px-3 rounded-lg transition-colors ${matches ? "bg-amber-50 border border-amber-200" : "hover:bg-muted/50"}`}>
