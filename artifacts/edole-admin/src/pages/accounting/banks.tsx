...
export default function BanksPage() {
  ...
  return (
    <AccountingShell
      ...
      actions={<Button onClick={() => setOpenBank(true)} className="bg-orange-600 hover:bg-orange-700"><Plus className="w-4 h-4 mr-2" />Nouveau compte</Button>}
    >
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
        {banks?.data.map((b) => (
          <Card key={b.id} className={`cursor-pointer transition ${selected === b.id ? "border-orange-500 ring-2 ring-orange-200" : ""}`} onClick={() => setSelected(b.id)}>
          ...
          <CardContent className="p-0">
            <div className="px-5 py-3 border-b bg-slate-50 flex justify-between items-center">
              ...
              <div className="flex gap-2">
                ...
                <Button size="sm" onClick={() => setOpenTx(true)} className="bg-orange-600 hover:bg-orange-700"><Plus className="w-3 h-3 mr-1" />Mouvement</Button>
              </div>
            </div>
      ...
      <Dialog open={openBank} onOpenChange={setOpenBank}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button onClick={() => createBank.mutate()} disabled={!bankForm.name || !bankForm.accountId || createBank.isPending} className="bg-orange-600 hover:bg-orange-700">Créer</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={openTx} onOpenChange={setOpenTx}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button onClick={() => createTx.mutate()} disabled={!txForm.label || !txForm.amount || createTx.isPending} className="bg-orange-600 hover:bg-orange-700">Enregistrer</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
                ...
                {recon?.transactions.map((t) => (
                  <div key={t.id} className="border rounded p-3 text-sm">
                    ...
                    <details className="mt-2">
                      <summary className="text-xs text-orange-600 cursor-pointer font-semibold">Rapprocher avec…</summary>
