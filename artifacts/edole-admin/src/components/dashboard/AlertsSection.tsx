      ...
      <div className="mt-3 space-y-2">
        {outstanding > 0 && (
          <Link href="/factures" className="flex items-center gap-3 p-3 rounded-lg border border-amber-200 bg-amber-50 hover:bg-amber-100 transition-colors">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-semibold text-foreground">Créances à encaisser</p>
              <p className="text-xs text-muted-foreground">{formatFCFA(outstanding)} en attente</p>
            </div>
        ...
        {overdueInvoices.map((inv: any) => (
          <Link key={inv.id} href="/factures"
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-semibold text-foreground truncate">Facture {inv.invoiceNumber || inv.number || `#${(inv.id || "").slice(0, 6)}`}</p>
              <p className="text-xs text-muted-foreground truncate">{formatFCFA(Number(inv.totalAmount || 0))} · {inv.clientName || "Client"} · échue le {shortDateFr(inv.dueDate)}</p>
            </div>
