...
router.get("/projects/stats", requirePermission("projects.read"), async (req, res) => {
  ...
  if (accessible !== null) {
    const projects = accessible.length === 0 ? [] : await db.select().from(projectsTable).where(and(eq(projectsTable.organizationId, req.authUser!.organizationId), isNull(projectsTable.deletedAt), inArray(projectsTable.id, accessible)));
    return res.json({
  ...
  const allProjects = await db.select().from(projectsTable).where(and(eq(projectsTable.organizationId, req.authUser!.organizationId), isNull(projectsTable.deletedAt)));
...
router.get("/projects/:id", async (req, res) => {
  ...
  const phases = await db.select().from(projectPhasesTable).where(and(eq(projectPhasesTable.organizationId, req.authUser!.organizationId), eq(projectPhasesTable.projectId, req.params.id)));
  const allTasks = await db.select().from(tasksTable).where(and(eq(tasksTable.organizationId, req.authUser!.organizationId), eq(tasksTable.projectId, req.params.id)));
