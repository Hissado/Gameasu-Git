...
router.get("/services", async (req, res) => {
  ...
  const conds: any[] = [isNull(servicesTable.deletedAt)];
...
router.get("/services/:id", async (req, res) => {
  const [s] = await db.select().from(servicesTable).where(eq(servicesTable.id, req.params.id)).limit(1);
...
router.put("/services/:id", requireManagerOrAbove, async (req, res) => {
  try {
    ...
    }).where(eq(servicesTable.id, req.params.id)).returning();
    if (!s) return res.status(404).json({ error: "Not found" });
    return res.json(s);
  } catch (e: any) {
    return res.status(400).json({ error: e.message });
...
router.delete("/services/:id", requireManagerOrAbove, async (req, res) => {
  await db.update(servicesTable).set({ deletedAt: new Date() }).where(eq(servicesTable.id, req.params.id));
  return res.status(204).send();
});

router.get("/services-stats/summary", async (_req, res) => {
  const rows = await db.select({
    total: sql<number>`count(*)`,
    active: sql<number>`count(*) filter (where is_active = true)`,
    avgPrice: sql<number>`coalesce(avg(unit_price), 0)`,
  }).from(servicesTable).where(isNull(servicesTable.deletedAt));
  return res.json({
