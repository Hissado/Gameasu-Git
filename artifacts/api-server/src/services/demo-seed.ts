...
const DEMO_USER_EMAILS = [
  "directeur@gameasu.com", "commercial@gameasu.com", "finance@gameasu.com",
  "operations@gameasu.com", "operations@gameasu.com", "rh@gameasu.com",
];
...
export async function seedDemo(opts: { force?: boolean } = {}): Promise<{ skipped?: boolean; counts?: Record<string, number>; message?: string }> {
  ...
  // ─── Admin existant ───
  const admin = (await db.select().from(usersTable).where(eq(usersTable.email, "admin@gameasu.com")).limit(1))[0];
  if (!admin) throw new Error("Compte admin@gameasu.com introuvable.");

  // ─── Utilisateurs supplémentaires ───
  const passwordHash = "demo2026";
  const userSpecs = [
    { email: "directeur@gameasu.com", firstName: "Kouassi", lastName: "Adjoumani", role: "manager" },
    { email: "commercial@gameasu.com", firstName: "Aminata", lastName: "Diallo", role: "manager" },
    { email: "finance@gameasu.com", firstName: "Salif", lastName: "Traoré", role: "collaborator" },
    { email: "operations@gameasu.com", firstName: "Moussa", lastName: "Konaté", role: "collaborator" },
    { email: "operations@gameasu.com", firstName: "Fatou", lastName: "Sangaré", role: "collaborator" },
    { email: "rh@gameasu.com", firstName: "Mariam", lastName: "Ouédraogo", role: "manager" },
  ];
  ...
  // ─── Collaborateurs ───
  const collabSpecs = [
    { firstName: "Kouassi", lastName: "Adjoumani", email: "directeur@gameasu.com", deptCode: "DIR", posCode: "DIR-GEN", baseSalary: "1200000" },
    { firstName: "Aminata", lastName: "Diallo", email: "commercial@gameasu.com", deptCode: "COM", posCode: "COM-DIR", baseSalary: "850000" },
    { firstName: "Salif", lastName: "Traoré", email: "finance@gameasu.com", deptCode: "FIN", posCode: "FIN-CPT", baseSalary: "650000" },
    { firstName: "Moussa", lastName: "Konaté", email: "operations@gameasu.com", deptCode: "OPS", posCode: "OPS-CHE", baseSalary: "550000" },
    { firstName: "Fatou", lastName: "Sangaré", email: "operations@gameasu.com", deptCode: "OPS", posCode: "OPS-CON", baseSalary: "480000" },
    { firstName: "Mariam", lastName: "Ouédraogo", email: "rh@gameasu.com", deptCode: "RH", posCode: "RH-DIR", baseSalary: "780000" },
  ...
  await db.insert(userClientAccessTable).values(
    clientIds.map(cid => ({ organizationId, userId: users["commercial@gameasu.com"], clientId: cid, accessLevel: "manager", grantedById: admin.id }))
  );
  await db.insert(userClientAccessTable).values(
    clientIds.slice(0, 4).map(cid => ({ organizationId, userId: users["operations@gameasu.com"], clientId: cid, accessLevel: "viewer", grantedById: admin.id }))
  );
  ...
  for (const e of engagementSpecs) {
    const [created] = await db.insert(clientServicesTable).values({
      ...
      ownerId: users["commercial@gameasu.com"],
  ...
  for (const p of projectSpecs) {
    const [created] = await db.insert(projectsTable).values({
      name: p.name, clientId: clientIds[p.clientIdx], managerId: users["operations@gameasu.com"],
  ...
  for (let i = 0; i < engagementIds.length; i++) {
    ...
    for (let j = 0; j < 3; j++) {
      const [parent] = await db.insert(tasksTable).values({
        ...
        assigneeId: users["operations@gameasu.com"],
  ...
  for (let i = 0; i < 3; i++) {
    ...
    await db.insert(inspectionsTable).values({
      rentalId: rental.id, type: "before", conductedById: users["operations@gameasu.com"],
    ...
    await db.insert(logisticsOperationsTable).values({
      ...
      responsibleId: users["operations@gameasu.com"],
    ...
    await db.insert(equipmentMovementsTable).values({
      ...
      quantity: 1, performedById: users["operations@gameasu.com"],
  ...
  for (const o of oppSpecs) {
    const [created] = await db.insert(opportunitiesTable).values({
      ...
      assignedToId: users["commercial@gameasu.com"],
  ...
  for (let i = 0; i < 8; i++) {
    await db.insert(activitiesTable).values({
      ...
      userId: users["commercial@gameasu.com"],
  ...
  await db.insert(marketingCampaignsTable).values([
    {
      ...
      recipientsCount: 142, sentCount: 138, failedCount: 4, createdBy: users["commercial@gameasu.com"],
    },
    {
      ...
      recipientsCount: 65, sentCount: 0, failedCount: 0, createdBy: users["commercial@gameasu.com"],
    },
  ] as any);
  counts.campaigns = 2;
  ...
  for (let i = 0; i < 4; i++) {
    ...
    const participants = [admin.id, users["operations@gameasu.com"], users["commercial@gameasu.com"]];
  ...
  for (let i = 0; i < 4; i++) {
    await db.insert(ticketsTable).values({
      ...
      createdById: users["commercial@gameasu.com"],
      assigneeId: admin.id,
    } as any);
  }
